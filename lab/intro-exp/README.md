...
- `--no-cache` — для `graph_traverse`: прямой ввод-вывод через `O_DIRECT`
  на Linux, `F_NOCACHE` на macOS, рекомендательный `O_DIRECT` на FreeBSD.
  На Linux адрес буфера, смещение и размер
  операции выравниваются по требованиям файловой системы (`statx`, если
  доступен; иначе консервативное выравнивание по странице/блоку). Формат файла
  менять не нужно: программа читает охватывающие блоки, а запись поля значения
  выполняет через read-modify-write с сохранением соседних байтов и исходной
  длины файла. Это увеличивает объём I/O; одновременная запись/усечение файла
  другими процессами не поддерживается. При ошибке открытия, настройки или I/O
  программа сообщает ошибку, а не повторяет операцию без запрошенного режима.
  На FreeBSD используются обычные `lseek` + `read`/`write`, без Linux-буфера
  выравнивания: согласно [FreeBSD open(2)](https://man.freebsd.org/cgi/man.cgi?query=open&sektion=2),
  `O_DIRECT` просит ОС минимизировать влияние данных на кэш. Семантика зависит
  от файловой системы; она может полностью игнорировать флаг. Успешный запуск
  не доказывает обход кэша, и считать этот режим эквивалентом Linux `O_DIRECT` нельзя.
  На остальных ОС `graph_traverse --no-cache` сообщает об отсутствии поддержки.
  Для `graph_traverse_mmap` на Linux, macOS и FreeBSD флаг даёт лишь рекомендации `madvise`: отображение
  по-прежнему использует page cache, гарантировать холодный кэш нельзя.
  Флаг не очищает системный кэш целиком и не отключает кэш накопителя; см.
  [`doc/experiments/environment.md`](../../doc/experiments/environment.md), раздел 6.

Для изучения исходников начните с `src/graph_traverse.c`: там остаются чтение
вершины и сам обход цепочки. Детали работы с файлом вынесены в `src/graph_io.h`:
`graph_read`/`graph_write` используют `lseek` + `read`/`write`, а на Linux с
`--no-cache` обрабатывают запрос по одному выровненному блоку. Один рабочий
буфер выделяется при открытии файла и освобождается при закрытии. Повторно
используется только память буфера: данные каждый раз заново читаются из файла.
...
- `--no-cache` в `graph_traverse_mmap` использует `MADV_SEQUENTIAL` перед
  обходом и `MADV_DONTNEED` после него. Это рекомендации, а не обход page cache;
  учтите влияние на readahead даже для случайной цепочки. В режиме записи
  перед `MADV_DONTNEED` выполняется `msync(MS_SYNC)`, стоимость которого входит
  во время обхода. Считать этот режим эквивалентом `O_DIRECT` нельзя;
