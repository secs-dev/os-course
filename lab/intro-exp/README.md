...
- `--no-cache` — для `graph_traverse`: прямой ввод-вывод через `O_DIRECT`
  на Linux, `F_NOCACHE` на macOS. На Linux адрес буфера, смещение и размер
  ...
  другими процессами не поддерживается. Если прямой I/O не поддерживается,
  программа сообщает ошибку, а не переключается молча на кэшируемый режим.
  Для `graph_traverse_mmap` флаг даёт лишь рекомендации `madvise`: отображение
