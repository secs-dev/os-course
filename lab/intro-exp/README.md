...
Экспериментально сравнить время работы приложений, моделирующих отдельные
паттерны нагрузки, характерные для баз данных: чтение и обновление связанных
записей в файле. В приложениях записи представлены вершинами графа, а обращения
к ним — обходом связей между вершинами.

Сравнение проводится при разных паттернах доступа к данным:
...
Оба приложения принимают одинаковый интерфейс командной строки:
...
- `--no-cache` — отключить системный кеш (почти, на самом деле эффект зависит от
  ОС и исследуемого приложения и не гарантирует холодный кэш во всех случаях.)

Детали реализации `--no-cache` можно почерпнуть в комментариях к
[`src/graph_io.h`](src/graph_io.h) и
...
- `--max-step-pages` — максимальное расстояние (в страницах памяти) между
  последовательно посещаемыми узлами, чтобы можно было задать высокую локальность.

```sh 
# пример высокой пространственной локальности
python3 graphgen.py -s 64M --seed 427 --topology chain -b 0.7 \
  --min-step-pages 0 --max-step-pages 1 -o graph-seq.bin
```
