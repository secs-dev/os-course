...
- `--no-cache` — сбрасывать файл из кэша ОС перед каждым обходом. На Linux кэш
  не обходится: каждый обход начинается с холодного кэша, но внутри обхода
  прочитанные страницы кэшируются как обычно (предпочитайте этот
